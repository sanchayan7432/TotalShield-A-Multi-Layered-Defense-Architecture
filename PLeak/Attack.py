...
class HotFlip:
    ...
    def init_triggers(self, trigger_token_length, init_trigger='', user_prefix=''):
        ...
        if self.target_model in ['opt', 'vicuna'] or 'llama' in self.target_model or 'deepseek' in self.target_model:
            init_tokens = init_tokens[1:]
            len_init -= 1
            len_user -= 1

        if len_init >= trigger_token_length:
            return np.asarray(init_tokens[:trigger_token_length])
        ...
        for idx in range(len(triggers)):
            if idx < len_init:
                ...
                while re.search("[^a-zA-Z0-9\s]", self.tokenizer.decode([t])):
                    t = np.random.randint(self.vocab_size)
                triggers[idx] = t
        return triggers
    ...
    def get_triggers_grad(self):
        ...
        if embedding_layer.weight.grad is None:
            print("WARNING: No gradient on embeddings")
            # return zero gradients per token to avoid crashing
            return torch.zeros((len(self.trigger_tokens), embedding_layer.weight.shape[1]), device=self.device)
        grad = embedding_layer.weight.grad[self.trigger_tokens]
        return grad
    ...
    def make_target(self, index, idx_loss, target_text, triggers):
        ...
        if self.target_model in ['opt', 'vicuna'] or 'llama' in self.target_model or 'deepseek' in self.target_model:
            ...
            encoded_target = encoded_target_text
        ...
        if max_len > getattr(self, "max_len", 0):
    ...
    def compute_loss(self, target_texts, trigger_tokens, idx_loss, require_grad=False):
        ...
        for index, text in enumerate(target_texts):
            ...
            outputs = self.model(lm_input, labels=label)
            loss = outputs.loss / len(target_texts)
            if require_grad:
                loss.backward()
            total_loss += loss.item()
        return total_loss

    # ------------------- HotFlip attack per token -------------------
    def hotflip_attack(self, averaged_grad, num_candidates=30):
        embedding_matrix = self.embedding_weight
        trigger_len = len(self.trigger_tokens)

        all_candidates = []
        for i in range(trigger_len):
            grad_i = averaged_grad[i]  # shape: embedding_dim
            grad_i = grad_i.unsqueeze(0)  # shape: 1 x embedding_dim
            gradient_dot_embedding_matrix = torch.matmul(grad_i, embedding_matrix.T)  # 1 x vocab_size
            gradient_dot_embedding_matrix *= -1  # minimize loss
            topk_vals, topk_ids = torch.topk(gradient_dot_embedding_matrix, num_candidates, dim=1)
            all_candidates.append(topk_ids.squeeze(0).detach().cpu().numpy())

        candidates_array = np.stack(all_candidates, axis=0)  # shape: (trigger_len, num_candidates)
        return candidates_array

    # ------------------- trigger replacement loop -------------------
    def replace_triggers(self, target_texts):
        ...
        while idx_loss <= self.max_len // self.step:
            ...
            while token_flipped:
                ...
                grad = self.get_triggers_grad()
                candidates = self.hotflip_attack(grad, num_candidates=30)

                best_trigger_tokens = deepcopy(self.trigger_tokens)
                for i in range(len(self.trigger_tokens)):
                    for cand in candidates[i]:
                        if re.search("[^a-zA-Z0-9\s]", self.tokenizer.decode([cand])):
                            continue
                ...
                if token_flipped:
                    print(f"Loss improved: {best_loss}, triggers: {self.decode_triggers()}")
                else:
...
# import torch                       # for othersexcept msphi
# import numpy as np
# from copy import deepcopy
# import re
# from util.template import TextTemplate
